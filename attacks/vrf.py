...
ap.add_argument("-p", required = True, type = int, default = 10,
...
def main():
    ...
    split = split_dataset(args["c"], args["p"], args["s"])
    print(
...
def split_dataset(
    ...
    for c in range(0,websites):
        for p in range(0,subpages):
            for s in range(0,samples):
                ID = to_file_label(c, p, s)
                test_index = int(split_list[c].split(",")[0])
                validation_index = int(split_list[c].split(",")[1])
                if p == test_index:
                    testing.append(ID)
